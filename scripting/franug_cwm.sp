...
#define DATA "3.1 private version for CS:S"
...
public int Menu_Handler2(Menu menu, MenuAction action, int client, int param2)
{
	switch (action)
	{
		case MenuAction_Select:
		{
			...
			{
				int weapon = Client_GetWeapon(client, client_w[client]);
				if(weapon != INVALID_ENT_REFERENCE)
					CWI_SetWeapon(weapon, -1, client);
			...
			else
			{
				...
				if(HasPermission(client, flag))
				{
					int weapon = Client_GetWeapon(client, client_w[client]);
					if(weapon != INVALID_ENT_REFERENCE)
						CWI_SetWeapon(weapon, PrecacheModel(cwmodel), client);
...
public tbasico6(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	if(HasPermission(client, flag)){
		int weapon2 = Client_GetWeapon(client, items);
		if(weapon2 != INVALID_ENT_REFERENCE)
			CWI_SetWeapon(weapon2, PrecacheModel(cwmodel), client);
	}
